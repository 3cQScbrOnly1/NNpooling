...
int Classifier::createAlphabet(const vector<Instance>& vecInsts) {
	...
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		int sparse_feat_num = feat.m_linear_features.size();

		for (int i = 0; i < sparse_feat_num; i++)
		{
			string cur_sparse_feat = feat.m_linear_features[i];
			m_feat_stats[cur_sparse_feat]++;
		}
...
void Classifier::extractFeature(Feature& feat, const Instance* pInstance) {
	...
	const vector<string>& words = feat.m_tweet_words;
...
void Classifier::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_driver._modelparams.linearAlpha.initial(m_feat_stats, m_options.featCutOff);
