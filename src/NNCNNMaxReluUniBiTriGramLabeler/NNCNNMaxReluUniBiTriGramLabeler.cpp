#include "NNCNNMaxReluUniBiTriGramLabeler.h"
...
int Classifier::createAlphabet(const vector<Instance>& vecInsts) {
	...
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		const vector<string> &feats = feat.m_linear_features;
		...
		int feats_num = feats.size();

		for (int i = 0; i < feats_num; i++) {
			string curFeat = feats[i];
	...
	cout << "Feat num: " << m_feat_stats.size() << endl;
